...
if __name__ == "__main__":
	...
	parser.add_argument('--company', '-c', dest='company_name', help='Name of company I am applying to', \
		 nargs='?', default="leoncyao@gmail.com")
	...
	parser.add_argument('--cover_letter', '-cl',  action='store_true', help='Flag to indicate whether to generate a cover letter with')
	args = parser.parse_args()

	print(args.role_name)
	...
	try:
		...
		message.set_content(
			f"Dear Hiring Manager,\n\nI am emailing to apply for the {args.role_name} position." + \
		...
		message["Subject"] = f"{args.role_name} Application"

		attachment_filename = "Leon_Yao___Resume___November___2023.pdf"
		...
		if args.cover_letter:
			attachment_filename = "cover_letter.pdf"
			type_subtype, _ = mimetypes.guess_type(attachment_filename)
			maintype, subtype = type_subtype.split("/")
			with open(attachment_filename, "rb") as fp:
				attachment_data = fp.read()
